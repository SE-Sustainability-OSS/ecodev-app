...
TIMEOUT = 20
BIG_TIMEOUT = 40
LOCAL = os.getenv('SELENIUM_LOCAL', 'false').lower() == 'true'
...
class PythonOrgSearchTest(SafeTestCase):
    ...
    def setUpClass(cls):
        ...
        selenium_host = 'localhost' if LOCAL else 'selenium'
        cls.driver = webdriver.Remote(command_executor=f'http://{selenium_host}:4444/wd/hub',
                                      options=options)
    ...
    def test_project_creation(self):
        # login
        print('logintoto')
        print(SETTINGS.api.user)
        print(SETTINGS.api.password)
        ...
        project_name_field = WebDriverWait(self.driver, TIMEOUT).until(
            EC.element_to_be_clickable((By.XPATH, "//input[@id='{\"index\":\"name\",\"type\":\"project-info-input\"}']")))
        project_name_field.send_keys('test_project')
        project_year_field = WebDriverWait(self.driver, TIMEOUT).until(
            EC.element_to_be_clickable((By.XPATH, "//input[@id='{\"index\":\"year\",\"type\":\"project-info-input\"}']")))
        project_year_field.send_keys('2022')

        create_project_button = WebDriverWait(self.driver, TIMEOUT).until(
            EC.element_to_be_clickable((By.XPATH, "//button[@id='{\"index\":\"project-info-save\",\"type\":\"button\"}']")))
